      ...
      private async withTimeout<T>(
        ...
        return await Promise.race<Promise<T>>([
          ...
          new Promise<T>((_, reject) =>
            setTimeout(
              () => reject(new Error(`Timeout ${ms}ms in ${label}`)),
              ms
            )
          ),
        ]);
      ...
      async onDidSendMessage(message: DebugProtocolMessage): Promise<void> {
        ...
        try {
          ...
          for (let attempt = 0; attempt < retries; attempt++) {
            try {
              // For entry events give a slightly larger initial settle delay
              if (attempt > 0 || isEntry) {
                const baseDelay = isEntry ? 75 : 50; // ms
                await new Promise(r => setTimeout(r, baseDelay * (attempt + (isEntry ? 1 : 0))));
              }
              if (!body.threadId || body.threadId === 0) {
                throw new Error('Invalid thread id on stopped event');
              }
              callStackData = await this.withTimeout(
                DAPHelpers.getDebugContext(session, body.threadId),
                2500,
                'getDebugContext'
              );
          ...
          if (!callStackData) {
            ...
            if (
              ...
              (/not paused/i.test(lastError.message) || /Invalid thread id/i.test(lastError.message))
            ) {
              logger.debug(
