...
describe('startDebuggingAndWaitForStop - capture-all (variableFilter omitted)', () => {
  ...
  it('capture action without variableFilter auto-captures variables and interpolates log message', async () => {
    ...
    assert.strictEqual(context.frame.line, targetLine, 'Did not pause at expected line for capture-all test');
    assert.ok(context.hitBreakpoint, 'hitBreakpoint missing');
    assert.strictEqual(context.hitBreakpoint?.action, 'capture');
    assert.strictEqual(
    ...
    assert.ok(
      Array.isArray(context.capturedLogMessages) && context.capturedLogMessages.length === 1,
      'Expected one captured log message'
    );
    assert.ok(
      /i=\d+/.test(context.capturedLogMessages![0]),
      `Log message interpolation missing expected variable value: ${context.capturedLogMessages![0]}`
    );
  });
});
